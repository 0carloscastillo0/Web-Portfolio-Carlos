...
projectRouter.post("/users/:userId/projects", authenticate, authorizeOwner("userId"), validate(paramsSchemas.userId, "params") , validate(projectCreateSchema, "body"), projectController.createProject);
...
projectRouter.put("/users/:userId/projects/:projectId", authenticate, authorizeOwner("userId"), validate(paramsSchemas.userProject, "params"), validate(projectUpdateSchema, "body"), projectController.updateProject);

// Delete project by ID with validation of projectId parameter
projectRouter.delete("/users/:userId/projects/:projectId", authenticate, authorizeOwner("userId"), validate(paramsSchemas.userProject, "params"), projectController.deleteProject);

// Upload images to a project (renamed from addImageToProject)
projectRouter.post("/users/:userId/projects/:projectId/images", authenticate, authorizeOwner("userId"), validate(paramsSchemas.userProject, "params"), uploadProjectImage.array("images",10), projectController.uploadImageProject);
